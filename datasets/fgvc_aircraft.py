...
class FGVCAircraftData(Dataset):
    def __init__(
        self,
        root,
        split,
...
class FGVCAircraftDataModule(BaseDataModule):
    def __init__(self, **params):
        super(FGVCAircraftDataModule, self).__init__(**params)

    def setup(self, stage: str):
        self.train_dataset = FGVCAircraftData(
            self.data_path,
            split="train",
            transform=self.train_transforms,
            split_file=self.split_file,
        )
        self.val_dataset = FGVCAircraftData(
            self.data_path,
            split="val",
...
if __name__ == '__main__':
    ...
    from augmentation.policies.fgvc_aircraft import TrainTransform, TestTransform

    # Get DATA_ROOT from environment or use default
    data_root = os.environ.get("DATA_ROOT", "./data")

    print("="*80)
    print("Testing FGVCAircraft Dataset")
    ...
    train_ds = FGVCAircraftData(root=f"{data_root}/FGVCAircraft", split="train", transform=train_aug)
    ...
    val_ds = FGVCAircraftData(root=f"{data_root}/FGVCAircraft", split="val", transform=val_aug)
    ...
    print("\n" + "="*80)
    print("✓ FGVCAircraft Dataset test completed successfully!")
    print("="*80)
