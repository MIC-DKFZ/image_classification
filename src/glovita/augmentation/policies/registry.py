...
User-facing selection happens through `glovita.configs.data`.
Policy metadata lives next to the actual augmentation implementation:

- shared 2D policies in `glovita.augmentation.policies.two_dim.defaults`
- shared 3D policies in `glovita.augmentation.policies.three_dim.defaults`
- dataset-specific custom policies in
  `glovita.augmentation.policies.dataset_specific.<dataset>`
"""
...
from glovita.augmentation.policies.metadata import TrainPolicySpec, TransformBuilder
from glovita.augmentation.policies.three_dim import defaults as defaults_3d
...
def _load_dataset_policy_module(dataset: str):
    ...
    try:
        return import_module(f"glovita.augmentation.policies.dataset_specific.{module_name}")
...
def resolve_policy_names(
    ...
    Dataset defaults now live in `glovita.configs.data`; the registry only checks
    ...
    if train_policy is None or test_policy is None:
        raise ValueError(
            ...
            "Set them in the dataset config under glovita.configs.data or override them via CLI."
        )
...
def build_transforms(
    ...
    **overrides,
    ...
    common_overrides = _compact_overrides(overrides)
    train_kwargs = dict(train_spec.default_kwargs)
    train_kwargs.update(common_overrides)
    train_kwargs.update(_compact_overrides(train_overrides or {}))
    test_kwargs = dict(common_overrides)
    test_kwargs.update(_compact_overrides(test_overrides or {}))
