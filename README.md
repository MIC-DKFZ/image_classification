...
- install the branch from nnssl that you need (so far the Eva Encoder from the raphirebase branch was used)
...
- create a config file with your default values, see cli_configs/model/eva_mae.yaml for an example

### Running the finetuning
You can do full finetuning `model.finetune_method=full` (default) or `model.finetune_method=linear_probing` (freezes encoder weights so that only head is trained).

In the following commands replace `model=eva_mae` with your model's config yaml name.

Hyperparameters used are saved in the respective config dataset yaml. To change them you can either adapt the config file directly or use the cmd, e.g. for changing the epochs you can do `trainer.max_epochs=100`, for changing the lr you can `model.lr=0.001`, for deactivating train transforms you can do `data.module.train_transforms=null` etc.

You can choose whether to use the cls_token for classification or to aggregate the token features instead by specifying `model.token_aggregation_method=cls_token` (default) or `model.token_aggregation_method=avg` or `sum` for the token features.


#### MRNet
`python main.py env=cluster model=eva_mae data=mrnet trainer.sync_batchnorm=True trainer.devices=2 model.pretrained=True model.chpt_path=<Path/to/checkpoint>`

`python main.py env=cluster model=eva_mae data=mrnet trainer.devices=1 model.pretrained=True model.finetune_method=linear_probing data.module.batch_size=128 trainer.accumulate_grad_batches=2 model.chpt_path=<Path/to/checkpoint>`

#### RSNA Spine
`python main.py env=cluster model=eva_mae data=rsna_spine trainer.sync_batchnorm=True trainer.devices=2 model.pretrained=True model.chpt_path=<Path/to/checkpoint>`

`python main.py env=cluster model=eva_mae data=rsna_spine trainer.devices=1 model.pretrained=True model.finetune_method=linear_probing data.module.batch_size=32 trainer.accumulate_grad_batches=8 model.chpt_path=<Path/to/checkpoint>`

### Results for training from scratch 
(all metrics are validation metrics averaged over 5 folds)
#### MRNet
`python main.py env=cluster model=eva_mae data=mrnet trainer.devices=2 model.pretrained=False`
